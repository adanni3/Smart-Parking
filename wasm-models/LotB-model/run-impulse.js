...
async function cropAndConvert(fullImagePath) {
  const cropped = await sharp(fullImagePath)
    .resize(96, 96, { fit: 'fill' }) // Edge Impulse expects exactly 96x96
    .grayscale()
    .raw()
    .toBuffer();
    const input = Uint8Array.from(cropped);
    console.log(input)

  console.log("Cropped buffer length:", cropped.length);

}


async function convertToHexArray(imagePath) {
  // Resize and ensure RGB format
  const { data, info } = await sharp(imagePath)
    .resize(96, 96, { fit: 'fill' })  // Resize to 96x96 (or your model size)
    .removeAlpha()
    .raw()
  ...
  for (let i = 0; i < data.length; i += 3) {
    ...
    hexArray.push(`0x${hex.toString(16).padStart(6, '0')}`);
  }
    let answer 
...
//     let result = classifier.classify(hexArray);
    
//         result = result.results[0].label;
//         answer = result
...
async function runClassification(hexArray) {
    let classifier = new EdgeImpulseClassifier();
     classifier.init();
    ...
    console.log('ClassificationB result:', result.results);
